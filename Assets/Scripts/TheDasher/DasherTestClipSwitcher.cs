...
namespace Kinex.TheDasher
{
    ...
    ///
    /// It feeds the RECORDED LANDMARK JSON (Assets/TheDasher/TestFeeds/*.json), not the .mp4, because
    ...
    /// </summary>
    public class DasherTestClipSwitcher : MonoBehaviour
    {
        ...
        void Awake()
        {
            ...
            for (int i = 0; i < ClipNames.Length; i++)
            {
                ...
                if (_feeds[i] == null)
                    Debug.LogWarning($"[DasherTestClipSwitcher] no landmark json for '{ClipNames[i]}' at {path}");
            }
        }
        ...
        void Update()
        {
            ...
            for (int i = 0; i < keys.Length; i++)
                if (keys[i].wasPressedThisFrame && _feeds[i] != null)
                {
                    ...
                    if (!_replayScored)
                    {
                        ...
                        var dir = FindObjectOfType<TheDasherDirector>();
                        if (dir != null) dir.BeginReplayTest();
                        _replayScored = true;
                    }
                    Debug.Log($"[DasherTestClipSwitcher] numpad{i + 1} → replay {ClipNames[i]} ({_feeds[i].frames.Length} frames)");
                }
            ...
            if (kb.numpad0Key.wasPressedThisFrame)
            {
                _active = -1;
                detector.StopReplay();
                Debug.Log("[DasherTestClipSwitcher] numpad0 → stop replay (live camera)");
            }
