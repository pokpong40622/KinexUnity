...
namespace Kinex.TheDasher
{
    ...
    /// </summary>
    public class DasherItem : MonoBehaviour
    {
        public enum Phase { Falling, Active, Resolved }

        public DasherKind Kind { get; private set; }
        ...
        public event Action<DasherItem> Arrived;
        /// <summary>Fired once when the active window runs out unresolved.</summary>
        public event Action<DasherItem> Expired;
        ...
        public void Init(DasherKind kind, int lane, Vector3 spawnPos, Vector3 endPos,
                         float fallSeconds, float windowSeconds, GameObject telegraph)
        {
            ...
            _spinDegPerSec = kind == DasherKind.Meteor ? 55f : (kind == DasherKind.Treasure ? 0f : 30f);
            ...
            switch (Stage)
            {
                case Phase.Falling:
                    ...
                    if (_t >= _fallSeconds)
                    {
                        ...
                        if (Kind == DasherKind.Meteor) Impact();
                    }
                    break;

                case Phase.Active:
                    // Treasure sits still; the ring bobs gently at hover height.
                    if (Kind == DasherKind.KickRing)
        ...
        {
            Stage = Phase.Resolved;
            KinexFx.PopBurst(transform.position + Vector3.up * 0.3f, DasherProps.MeteorOrange, 36);
        ...
        public void Collect(Transform flyTo)
        {
            ...
            Stage = Phase.Resolved;
            KillTelegraph();
            KinexFx.PopBurst(transform.position + Vector3.up * 0.4f, DasherProps.StarGold, 30);
        ...
        public void Kicked(int fromLane)
        {
            ...
            Stage = Phase.Resolved;
            KillTelegraph();
            KinexFx.PopBurst(transform.position, DasherProps.CyanGlow, 34);
