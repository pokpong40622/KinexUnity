...
{
    /// <summary>
    /// Procedural placeholder props for TheDasher — meteor, treasure crate, kick ring,
    ...
    /// </summary>
    public static class DasherProps
    {
        ...
        // Cached materials — every prop builder below used to call PropMeshes.Mat/MatUnlit
        // fresh on EVERY spawn (每 beat), leaking a new Material instance each time since
        // nothing ever destroys them. These run for a fixed small palette, so build once and
        // reuse. TelegraphRing's glow varies by caller-supplied color, so it's a small dictionary
        // cache instead of a single field.
        static Material s_MeteorRock, s_MeteorLava, s_TreasureGold, s_TreasureSeam,
            s_TreasurePallet, s_TreasureJet, s_KickGlow, s_GrabberMetal, s_GrabberAccent,
            s_TelegraphDark, s_ParticleMat;
        static readonly System.Collections.Generic.Dictionary<Color, Material> s_TelegraphGlow = new();

        // ---- Meshy GLB models (Resources/DasherModels/*), with the procedural builders below as
        ...
        {
            var prefab = Resources.Load<GameObject>("DasherModels/" + name);
        ...
        public static GameObject Meteor()
        {
            ...
            var rock = s_MeteorRock ??= PropMeshes.Mat(new Color(0.16f, 0.13f, 0.12f), 0f, 0.25f);
            var lava = s_MeteorLava ??= PropMeshes.Mat(MeteorOrange, 0f, 0.4f, MeteorOrange * 3.5f);
        ...
        static Material ParticleMat()
        {
            ...
            return s_ParticleMat = new Material(shader);
        }
        ...
        public static GameObject Treasure()
        {
            ...
            var gold = s_TreasureGold ??= PropMeshes.Mat(new Color(0.85f, 0.63f, 0.18f), 0.7f, 0.75f);
            var seam = s_TreasureSeam ??= PropMeshes.Mat(StarGold, 0.2f, 0.5f, StarGold * 3.2f);
            var pallet = s_TreasurePallet ??= PropMeshes.Mat(new Color(0.18f, 0.20f, 0.28f), 0.5f, 0.55f);
            var jet = s_TreasureJet ??= PropMeshes.Mat(CyanGlow, 0f, 0.4f, CyanGlow * 2.6f);
        ...
        public static GameObject KickRing(TMP_FontAsset thaiFont)
        {
            ...
            var glow = s_KickGlow ??= PropMeshes.Mat(CyanGlow, 0f, 0.5f, CyanGlow * 3.8f);
            const int orbs = 12;
        ...
        public static GameObject GrabberTool()
        {
            ...
            var metal = s_GrabberMetal ??= PropMeshes.Mat(new Color(0.55f, 0.58f, 0.66f), 0.85f, 0.8f);
            var accent = s_GrabberAccent ??= PropMeshes.Mat(CyanGlow, 0.3f, 0.6f, CyanGlow * 2.4f);
        ...
        public static GameObject TelegraphRing(Color color)
        {
            ...
            var dark = s_TelegraphDark ??= PropMeshes.MatUnlit(new Color(0f, 0f, 0f, 0.55f));
            if (!s_TelegraphGlow.TryGetValue(color, out var glow))
                s_TelegraphGlow[color] = glow = PropMeshes.Mat(color, 0f, 0.5f, color * 3.0f);
            ...
            // Was 20 — halves the per-spawn draw-call/GameObject count, still reads as a ring.
            const int segs = 10;
            ...
            for (int i = 0; i < segs; i++)
            {
                ...
                Primitive(PrimitiveType.Cube, $"Seg{i}", ring.transform,
                    ...
                    new Vector3(0.20f, 0.02f, 0.045f), glow, // widened to fill the gap from half the segments
