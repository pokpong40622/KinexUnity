...
{
    /// <summary>
    /// Tiny UI-only helper for the 3 difficulty buttons (ง่าย / ปกติ / ยาก) on the AstroStance intro
    /// panel. Built and wired by AstroStanceUIBuilder. Exists only because UnityEditor.Events.
    ...
    /// AstroStanceDirector.SetDifficulty takes AstroDifficulty — C# won't convert that method group to
    ...
    /// for every other button in AstroStanceUIBuilder. This component also owns the "which one is
    /// selected" tint — simplest place for it since it already needs references to all 3 button Images.
    /// </summary>
    public class AstroDifficultySelector : MonoBehaviour
    {
        public AstroStanceDirector director;
        public Image easyBg;
        ...
        /// call outside Play mode too — AstroStanceUIBuilder calls this once right after wiring so the
        /// Scene view already shows the right button highlighted before anyone presses Play.
        /// </summary>
        public void ApplyInitialHighlight()
        {
            var d = director != null ? director.CurrentDifficulty : AstroDifficulty.Normal;
            Highlight(d);
        }

        public void SelectEasy() => Select(AstroDifficulty.Easy);
        public void SelectNormal() => Select(AstroDifficulty.Normal);
        public void SelectHard() => Select(AstroDifficulty.Hard);

        void Select(AstroDifficulty d)
        {
            if (director != null) director.SetDifficulty(d);
            Highlight(d);
        }

        void Highlight(AstroDifficulty d)
        {
            SetTint(easyBg, d == AstroDifficulty.Easy);
            SetTint(normalBg, d == AstroDifficulty.Normal);
            SetTint(hardBg, d == AstroDifficulty.Hard);
        }
