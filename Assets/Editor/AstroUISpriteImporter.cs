...
{
    /// <summary>
    /// Applies the import settings from Assets/AstroStance/UI/manifest.json (contract:
    ...
    /// </summary>
    public static class AstroUISpriteImporter
    {
        const string UiFolder = "Assets/AstroStance/UI/";
        ...
        [MenuItem("Kinex/Import AstroStance UI Sprites")]
        public static void Import() => Debug.Log("[AstroUISpriteImporter] " + Run());
        ...
        public static string Run()
        {
            ...
            {
                Debug.LogWarning($"[AstroUISpriteImporter] Failed to parse {ManifestPath}: {e.Message}");
            ...
            foreach (var entry in manifest.sprites)
            {
                ...
                if (!File.Exists(path))
                {
                    Debug.LogWarning($"[AstroUISpriteImporter] Missing PNG (skipped): {path}");
                    missing++;
                    continue;
                }
                ...
                if (importer == null)
                {
                    Debug.LogWarning($"[AstroUISpriteImporter] No TextureImporter for: {path}");
                    missing++;
                    continue;
                }
