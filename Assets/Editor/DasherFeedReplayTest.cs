...
{
    /// <summary>
    /// Replays REAL recorded pose feeds through the C# motion detectors — the TheDasher
    ...
    /// live in Assets/TheDasher/TestFeeds/. Feed facts this test encodes:
    ...
    /// </summary>
    public static class DasherFeedReplayTest
    {
        const string FeedDir = "Assets/TheDasher/TestFeeds";
        ...
        [MenuItem("Kinex/Run TheDasher Feed Replay Test")]
        public static void Run()
        {
            ...
            // The DasherSitClassifier must ONLY fire on a real sit — never on a step or a kick.
        ...
        static void TestSitStand(string name)
        {
            ...
            // DasherSitClassifier is calibration-free: no baseline pass, and it boots in Standing
            // so a standing player yields NO synthetic stand before the real sit→stand rep.
            var sit = new DasherSitClassifier();
            var kp = new Vector2[17];
            var conf = new float[17];
            float dt = 1f / Mathf.Max(feed.fps, 1f);
