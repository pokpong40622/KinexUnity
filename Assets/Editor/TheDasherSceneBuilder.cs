...
{
    /// <summary>
    /// One-click / batchmode builder for Assets/Scenes/TheDasherScene.unity: a BEHIND-the-
    ...
    /// view faces the avatar away from the camera), the DasherSpawner, and the fully wired
    /// TheDasherDirector (+ result bridge). Idempotent: reopens the scene and rebuilds its own
    /// roots by name. Ends by appending the scene to Build Settings and rendering a 927x1427
    /// screenshot to the job scratchpad.
    /// </summary>
    public static class TheDasherSceneBuilder
    {
        const string ScenePath = "Assets/Scenes/TheDasherScene.unity";
        const string PostProfilePath = "Assets/TheDasher/TheDasherPost.asset";
        ...
        const string ScreenshotPath =
            "C:/Users/Admin/AppData/Local/Temp/claude/D--Unity-project-Kinex/552c1cb4-f9f4-4e3c-b616-07a08892f6bc/scratchpad/thedasher_scene.png";
        ...
        [MenuItem("Kinex/Build TheDasher Scene")]
        public static void BuildScene()
        {
            ...
            {
                Debug.LogWarning($"[TheDasherSceneBuilder] Character prefab missing for id '{CharacterId}'.");
            }
            ...
            if (spawner.thaiFont == null)
                Debug.LogWarning($"[TheDasherSceneBuilder] Font not found: {ThaiSemiPath}");
            ...
            {
                Debug.LogWarning($"[TheDasherSceneBuilder] screenshot failed — scene still saved: {e.Message}");
            }
            finally
            {
                canvas.renderMode = RenderMode.ScreenSpaceOverlay;
            ...
            Debug.Log($"[TheDasherSceneBuilder] Scene built + saved: {ScenePath}. Screenshot: {ScreenshotPath}");
        }
        ...
        {
            if (!AssetDatabase.IsValidFolder("Assets/TheDasher"))
                AssetDatabase.CreateFolder("Assets", "TheDasher");
