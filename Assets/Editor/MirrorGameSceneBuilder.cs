...
{
    /// <summary>
    /// One-click / batchmode builder for Assets/Scenes/MirrorGameScene.unity: camera + warm/cool
    /// lights + ACES post volume, the runtime-built Magic Mirror studio stage, the MediaPipe-driven
    /// player avatar (same gotcha fixes as BattleGameSceneBuilder), the portrait UI canvas with
    /// camera feed, and the fully wired MirrorGameDirector (+ result bridge + TTS voice). Idempotent:
    /// reopens the scene and rebuilds its known roots by name. Ends by appending the scene to Build
    /// Settings and rendering a 927x1427 screenshot (with a preview ghost + zones) to the scratchpad.
    /// </summary>
    public static class MirrorGameSceneBuilder
    {
        const string ScenePath = "Assets/Scenes/MirrorGameScene.unity";
        const string PostProfilePath = "Assets/MirrorGame/MirrorGamePost.asset";
        ...
        const string ScreenshotPath =
            "C:/Users/Admin/AppData/Local/Temp/claude/D--Unity-project-Kinex/3b7b0985-45db-45d2-bfa5-d7f306e07247/scratchpad/mirrorgame_scene.png";
        const int ShotW = 927, ShotH = 1427;
        const float AvatarHeightMeters = 1.7f;
        const int PreviewPoseIndex = 12; // an arm-reach pose so the ghost limbs + zones read clearly

        static readonly string[] OwnedRoots =
        {
            "Main Camera", "KeyLight", "FillLight", "RimLight", "GlobalVolume",
            "MirrorStage", "PlayerCharacter", "Canvas", "EventSystem", "Director", "~TempEnv",
        };

        [MenuItem("Kinex/Build Mirror Game Scene")]
        public static void BuildScene()
        {
            ...
            cam.allowHDR = true; // ghost/mirror glow rely on HDR values clearing the bloom threshold
            camGo.AddComponent<AudioListener>();
            cam.GetUniversalAdditionalCameraData().renderPostProcessing = true;

            // ---- Warm key light (studio spotlight) — casts real shadows onto the wood floor. ----
            ...
            // ---- Cool fill on the camera side so the pose-critical front stays readable. ----
            ...
            // ---- Warm rim behind the avatar (edge glow, catches the mirror mood). Also the only
            // light that faces the back/side walls front-on, so its intensity doubles as "how much
            // the studio walls read instead of falling to black" — bumped from 0.55 after the
            // walls/windows/skirting added in the graphics pass rendered almost indistinguishable
            // from the camera's dark clear colour. ----
            ...
            // ---- Post-processing volume (ACES + bloom for the ghost/mirror glow). ----
            ...
            // ---- Runtime-built studio stage. ----
            var stageGo = new GameObject("MirrorStage");
            stageGo.AddComponent<MirrorStage>();
            ...
            if (prefab != null)
            {
                ...
                var shirtTone = new Color(0.62f, 0.42f, 0.68f);   // soft violet — matches the studio
            ...
            {
                Debug.LogWarning($"[MirrorGameSceneBuilder] Character prefab missing for id '{CharacterId}'.");
            ...
            var director = directorGo.AddComponent<MirrorGameDirector>();
            directorGo.AddComponent<MirrorResultBridge>();
            ...
            // ---- UI (chained, like FruitGameSceneBuilder → FruitGameUIBuilder). ----
            Debug.Log("[MirrorGameSceneBuilder] " + MirrorGameUIBuilder.BuildUI());

            // ---- Build Settings: append only (Boot stays index 0). ----
            AppendToBuildSettings(ScenePath);

            // ---- Framing screenshot: temp studio env + a preview ghost/zones (both runtime-only). ----
            var tempEnv = new GameObject("~TempEnv");
            MirrorStage.BuildStage(tempEnv.transform);

            MirrorOutline preview = null;
            ...
            if (model != null && ghostRig != null && poseData != null)
            {
                preview = MirrorOutline.Create(model.transform, ghostRig, poseData, PreviewPoseIndex);
                if (preview != null)
                {
                    preview.SetAlphaImmediate(1f);
                    var g = new Color(0.32f, 0.9f, 0.42f, 0.95f);
                    var a = new Color(1f, 0.75f, 0.25f, 0.95f);
                    var r = new Color(0.92f, 0.32f, 0.32f, 0.9f);
                    preview.ApplyZones(new[] { g, g, a, a, r, r, g, g, g, g }, 1f);
                    preview.EditorPreviewLayout(cam);
                }
            }
            ...
            if (preview != null)
            {
                var holder = preview.ZonesHolder;
            ...
            Debug.Log($"[MirrorGameSceneBuilder] Scene built + saved: {ScenePath}. Screenshot: {ScreenshotPath}");
        }
        ...
        {
            if (!AssetDatabase.IsValidFolder("Assets/MirrorGame"))
                AssetDatabase.CreateFolder("Assets", "MirrorGame");
            ...
            // Tuned empirically by pixel-sampling the rendered screenshot: threshold 0.9 + scatter
            // 0.78 + intensity 0.95 produced a mathematically-above-threshold but PERCEPTUALLY
            // invisible bloom (the small per-pixel excess got smeared too thin by the wide scatter
            // kernel). A tighter kernel + stronger intensity makes the ghost/mirror/frame glow read
            // clearly while a 0.75 threshold still excludes the plain diffuse-lit floor/walls.
            ...
            smh.shadows.Override(new Vector4(0.8f, 0.85f, 1.1f, 0.1f));   // cool night shadows
            smh.highlights.Override(new Vector4(1.12f, 1.0f, 0.82f, 0.12f)); // warm highlights
        ...
        static void WarnIfNoPostProcessData()
        {
            ...
            if (!anyPost)
                Debug.LogWarning("[MirrorGameSceneBuilder] WARNING: no UniversalRendererData has Post Process Data assigned.");
        }
