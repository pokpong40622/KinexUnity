...
{
    /// <summary>
    /// One-click builder for the TheDasher UI — same procedural-canvas + SerializedObject-wiring
    /// pattern as TempleHuntUIBuilder. Re-runnable: destroys its own panels by name and rebuilds,
    /// then wires every TheDasherDirector field. Bright cream glass cards with DARK ink text
    ...
    /// </summary>
    public static class TheDasherUIBuilder
    {
        const float FW = 927f, FH = 1427f;

        // Palette (matches TheDasherSceneBuilder / DasherProps where it overlaps).
        ...
        {
            var sprite = AssetDatabase.LoadAssetAtPath<Sprite>("Assets/TheDasher/UI/" + name + ".png");
            if (sprite == null && _warnedMissingSprites.Add(name))
                Debug.LogWarning($"[TheDasherUIBuilder] Sprite not found, using procedural fallback: {name}.png");
            return sprite;
        }

        [MenuItem("Kinex/Build TheDasher UI")]
        public static void Build() => Debug.Log("[TheDasherUIBuilder] " + BuildUI());

        public static string BuildUI()
        {
            var director = Object.FindAnyObjectByType<TheDasherDirector>();
            if (director == null) return "No TheDasherDirector in the open scene. Run the scene builder first.";
            ...
            // art isn't built yet.
            var introBg = Ui("thedasherstartpagebackground");
            if (introBg != null)
            {
            ...
            // DasherDifficultySelector (Assets/Scripts/TheDasher/DasherDifficultySelector.cs) is a
            ...
            // SetDifficulty takes DasherDifficulty — that method-group-to-delegate conversion doesn't
            ...
            var diffSelector = diffSelectorGo.AddComponent<DasherDifficultySelector>();
            ...
            return "TheDasher UI rebuilt and wired to TheDasherDirector.";
        }
        ...
        {
            var tex = new Texture2D(4, 4, TextureFormat.RGBA32, false) { name = "DasherFeedPlaceholder" };
        ...
        static TMP_FontAsset LoadFont(string path)
        {
            ...
            if (f == null) Debug.LogWarning($"[TheDasherUIBuilder] Font not found: {path} (using TMP default)");
            return f;
        }
        ...
        static void Assign(SerializedObject so, string field, Object value)
        {
            ...
            else Debug.LogWarning($"[TheDasherUIBuilder] Director field not found: {field}");
        }
        ...
        {
            var p = so.FindProperty(field);
            if (p == null) { Debug.LogWarning($"[TheDasherUIBuilder] Director field not found: {field}"); return; }
